...
#ifndef LANEMARK3D_LANEDETECTIONMODULE_H
#define LANEMARK3D_LANEDETECTIONMODULE_H
...
public:
    ...
    std::vector<int> pointsLeftX={};
    std::vector<int> pointsRightX={};
    double headingDegree=0.0;
     ...
     */
    bool fitPoly(const std::vector<cv::Point>& src, cv::Mat& dst, int order);
    ...
    int pointsNum=10;
    std::vector<int> getLanePoints(Lane& lane1, cv::Mat inv);

    std::vector<int> getMedianPoints(std::vector<int> lane1points,std::vector<int> lane2points,int len);
     ...
     */
    bool detectLane(cv::Mat framem,int num);
...
#endif //LANEMARK3D_LANEDETECTIONMODULE_H
